import {
  BattleMap, Combatant, MapCell, MapProp, Participant, PropKind,
} from './board.models';

/**
 * A dungeon level, as data.
 *
 * <p><b>Written once and used twice</b>, which is the point of it living here
 * rather than inside the preview component. The preview renders it without a
 * server; the encounters page POSTs it to one, so a DM's first board is a
 * furnished level rather than an empty twenty-by-twenty square. The same rows
 * either way — if the preview draws it, the server can store it.
 *
 * <p>That is also the strongest check the board has that the client and the
 * server agree about what a board *is*: a fixture the server rejects is a
 * fixture that was lying.
 */

export const SAMPLE_NAME = 'The undercroft of Elsmere Keep';
...
export const SAMPLE_WIDTH = TERRAIN[0].length;
export const SAMPLE_HEIGHT = TERRAIN.length;
 ...
 * middle of row 1.
 */
export function worldRow(row: number): number {
  return SAMPLE_HEIGHT - 1 - row;
}
...
export function buildLevel(): MapCell[] {
  const cells: MapCell[] = [];
 ...
 */
export function furnish(): MapProp[] {
  const props: MapProp[] = [];
...
export function toCombatant(c: CastMember): Combatant {
  return {
    id: `c-${c.id}`,
    statBlockId: `sb-${c.id}`,
...
/**
 * The level as a map, ready either to draw or to POST.
 *
 * <p>Exactly the shape `BattleMapRequest` wants on the server, minus the `id`
 * the server assigns — so the preview's board and a created encounter's board
 * are built from one description, and neither can quietly drift from the other.
 */
export function sampleMap(): Omit<BattleMap, 'id'> {
  return {
    width: SAMPLE_WIDTH,
    height: SAMPLE_HEIGHT,
    cellFeet: 5,
    defaultTerrain: 'FLOOR',
    // Dim by default and lit where somebody put a torch, rather than the other
    // way round. A dungeon is dark; light is the exception and should have to
    // be placed.
    defaultLight: 'DIM',
    cells: buildLevel(),
    props: furnish(),
...
export function toParticipant(c: CastMember): Participant {
