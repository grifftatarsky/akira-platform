...
| 2.6 | **Shadows** — 4.03 ms for two cascades at 2048 | — | — | **examined 2026-09-13. It splits 1.8 receiving / 2.3 casting, and 1.2 of the casting is one tree.** Shadows off 26.4 ms; on with an empty caster list 28.2; all casters 30.5. The caster list is 4.93M triangles and `scan-island_tree_02-0` alone is **3.22M of them** — 403k triangles, eight instances, into every cascade every frame, for a shadow perhaps forty texels across. Dropping it saves 1.2 ms and its shadow is visibly missed. A decimated shadow proxy for that one scan is the ~1 ms; the stones are 0.1 ms and not worth touching |
