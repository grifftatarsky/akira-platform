...
| 2.6 | **Shadows** — 4.03 ms for two cascades at 2048 | — | — | **examined, and a proxy measured 2026-09-13.** 1.8 ms receiving / 2.3 casting, and `scan-island_tree_02-0` is 3.22M of the 4.93M caster triangles. A decimated caster — every triangle in the top 3% by area, plus a hashed fraction of the leaves — **saves 1.0 ms at 30% of the leaves** (29.1 → 28.1; removing the tree entirely is 27.7). Decimating *thins* the shadow rather than only cheapening it, because the leaves are alpha-test cut-outs. 12% saves 1.2 ms but the shadow is then closer to absent than to real. Accepted on the look at 30%; not yet wired into `standing.ts` |
